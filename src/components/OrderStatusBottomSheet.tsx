...
>(({ orderStatusArray }, ref) => {
  ...
  const snapPoints = useMemo(() => {
    ...
    const itemHeight = 80; // Altura aproximada por item
    const maxItems = 6; // Máximo de items visibles sin scroll
    ...
    const contentHeight =
      Math.min(orderStatusArray.length, maxItems) * itemHeight + baseHeight;
    const screenPercentage = Math.min(
      Math.max((contentHeight / 800) * 100, 30),
      80
    ); // Entre 30% y 80%
        ...
        <View className="flex-1 ml-4">
          <View className="flex-row items-center justify-between mb-2">
            ...
            <Text className="text-xs font-medium text-gray-500">
              {formatOrderDate(status.created_at)}
            </Text>
          </View>
        ...
        {!orderStatusArray || orderStatusArray.length === 0 ? (
          ...
          >
            {orderStatusArray.map((status, index) => (
              <StatusItem
                key={`${status.status}-${status.created_at}-${index}`}
                status={status}
                index={index}
